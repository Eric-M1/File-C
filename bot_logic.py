...
def get_tags_by_number(number: str) -> str:
    ...
    if not respons:
        return f'По "{number}" Нечего не нашёл'
    else:
...
def get_numbers_by_name(name: str) -> str:
    try:
        ...
        return f'🙎🏻‍♂️ - На "{name.title()}" Нечего не найдено'
    else:
        user_name = []
        if not respons:
            return f'🙎🏻‍♂️ - На "{name.title()}" Нечего не найдено'
...
def get_inn(user_name):
    ...
    try:
        ...
        if first_choice["n"] != user_name.upper():
...
def all_errors_f():
    all_errors = open('help_files/errors_file.txt', 'r').readlines()
